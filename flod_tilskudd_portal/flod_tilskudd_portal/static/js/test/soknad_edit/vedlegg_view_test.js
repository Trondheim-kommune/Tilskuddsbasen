...
(function (ns) {
    ...
    buster.testCase("VedleggViewTest", {
        ...
        "Test that no file is saved when no file is selected": function () {
            var model = new TestSoknadModel({vedlegg: []});
            assert.equals(0, model.get("vedlegg").length);

            var testedView = new ns.VedleggFilView({model: model, collection: new Backbone.Collection(model.get("vedlegg")), parentObject: "soknad"});
            testedView.render();
