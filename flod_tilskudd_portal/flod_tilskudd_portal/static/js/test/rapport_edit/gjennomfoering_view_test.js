...
(function (ns) {
    ...
    buster.testCase("GjennomforingViewTest", {

        setUp: function () {
            setup("/rapport_edit/content_edit_gjennomforing");
        },
        ...
        "Test that model and view are compatible": function () {
            refute.exception(function () {
                ...
                var testGjennomforingView = new ns.RapportGjennomforingView({
                    model: model,
                    collection: new Backbone.Collection(model.get("arrangement"))
                });
                var res = testGjennomforingView.render();
            });
        },
        ...
        "Test that events are inherited from base class": function () {
            ...
            var testedView = new ns.RapportGjennomforingView({model: model, collection: new Backbone.Collection(model.get("arrangement"))});
        ...
        "Test that start date updates end date if end date is less than start date": function () {
            var model = new TestRapportModel({'id':1, 'soknad_id':1, arrangement: [{id: "", sted: "", tidspunkt: "", start_dato: "", slutt_dato: ""}]});
